...
def create(db: Session, order_detail):
    # Create a new instance of the details model with the provided data
    db_order_detail = models.OrderDetail(
        sandwich_id=order_detail.sandwich_id,
        amount=order_detail.amount
    )
    # Add the newly created details object to the database session
    db.add(db_order_detail)
    # Commit the changes to the database
    db.commit()
    # Refresh the Order object to ensure it reflects the current state in the database
    db.refresh(db_order_detail)
    # Return the newly created details object
...
def update(db: Session, order_detail_id: int, order_detail):
    # Query the database for the specific details to update
    ...
    # Commit the changes to the database
    db.commit()
    # Return the updated details record
